...
from motioncap.raft_motion import (
    load_raft, compute_residual, compute_residual_and_flow_small, _to_tensor,
)
from motioncap.tracker import (
    decode_heatmap, run_tracker, Track, TrackPoint,
    RichDetection, TemporalFilter, detect_blobs,
)
...
def _load_heatmaps_from_pb(
    ...
    for resp in responses:
        if resp.tracks:               # skip the summary record
            continue
...
def main() -> None:
    ...
    if not heatmaps:
        ...
        for idx, frame_curr in enumerate(tqdm(frames, desc="Normalise", unit="frame")):
            ...
            heatmaps.append(_to_heatmap_u8(residual, global_scale, res_thresh, blur_kernel))
    ...
    if regen_raft and feat_enabled and rgb_cache and model is not None:
        ...
        with torch.no_grad():
            for fidx, (hm, rgb) in enumerate(
                tqdm(zip(heatmaps, rgb_cache), total=total,
                     desc="Features", unit="frame")
            ):
                ...
                for det in blobs:
                    ...
                    frame_rich.append(RichDetection(
                        centroid=det.centroid, area=det.area, bbox=det.bbox,
                        flow_vec=flow_vec, appearance=desc,
                    ))
                rich_detections.append(frame_rich)

        elapsed_feat = time.time() - t_feat
        print(f"Feature extraction done in {elapsed_feat:.1f}s "
    ...
    for idx in range(total):
        ...
        resp.stabilization_confidence  = 1.0 if (idx < len(max_raws) and max_raws[idx] > 0) else 0.0
        resp.heatmap.heatmap_data      = encode_heatmap(heatmaps[idx])
        resp.heatmap.max_motion_raw    = float(max_raws[idx]) if idx < len(max_raws) else 0.0
        batch.append(resp)
        if len(batch) >= 50:
    ...
    summary = _encode_tracks_record(tracks, total)
    _motion_io.write_file(out_path, [summary])

    print(f"→ {out_path.name}  ({total} frames, {len(tracks)} track(s))")

    # ── Optional video ────────────────────────────────────────────────────────

    if not args.output_video:
        print("\nDone.  Pass --output-video to also write a visualization.")
        return
