...
def render_pose_panel(
    ...
    Table corners/midline are projected from rvec/tvec (table PnP).
    Net rectangle is projected from net_rvec/net_tvec (independent net PnP)
    using canonical_net_local_mm; falls back to rvec/tvec + canonical_net_strip_mm
    if no independent net pose is supplied.

    Separate failure lines are shown for table and net when either PnP failed.
    """
    img = rgb_bgr.copy()
    if mask_overlays:
    ...
    if net_rvec is not None and net_tvec is not None:
        ...
        cv2.putText(img, "net PnP FAILED", (10, y), font, scale, COLOR_OFF, thick)
