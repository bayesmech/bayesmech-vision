...
class CameraFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewModel.isRecording.collect { recording ->
                ...
                // Re-evaluate send button: requires both recording and non-blank text
                val hasText = !binding.messageInput.text.isNullOrBlank()
                binding.sendButton.isEnabled = recording && hasText
                binding.sendButton.alpha = if (recording && hasText) 1f else 0.4f
                if (recording) {
        ...
        binding.sendButton.setOnClickListener {
            ...
            if (text.isNotBlank() && viewModel.isRecording.value) {
        ...
        binding.messageInput.addTextChangedListener(object : android.text.TextWatcher {
            ...
            override fun afterTextChanged(s: android.text.Editable?) {
                val canSend = !s.isNullOrBlank() && viewModel.isRecording.value
    ...
    private fun toggleFullscreen() {
        ...
        if (isFullscreen) {
            ...
            binding.fullscreenButton.setImageResource(com.bayesmech.vision.R.drawable.ic_fullscreen_exit)
        } else {
            binding.chatPanel.visibility = View.VISIBLE
            binding.recordButton.visibility = View.VISIBLE
            binding.fullscreenButton.setImageResource(com.bayesmech.vision.R.drawable.ic_fullscreen)
        }
    }

    private fun updateRecordButtonState(isRecording: Boolean) {
        if (isRecording) {
            binding.recordButton.setBackgroundResource(com.bayesmech.vision.R.drawable.record_button_recording)
            binding.liveBadge.text = "● REC"
            binding.liveBadge.visibility = View.VISIBLE
            startPulse()
        } else {
            stopPulse()
            binding.recordButton.setBackgroundResource(com.bayesmech.vision.R.drawable.record_button_idle)
